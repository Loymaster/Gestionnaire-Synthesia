...
(function () {
  ...
  /* ═══════════════════════════════════════════════════════════════════
     ...
     ═══════════════════════════════════════════════════════════════════ */
  var API = "https://script.google.com/macros/s/AKfycbyOp-cxko4ioh_IcvBirfQlUF6XeTlt_MgKVna7kpQTW31d4J2nmuwhBm0xN1b-fr52EQ/exec";
  /* Exemple :
  ...
  /* Le dépôt peut transporter jusqu'à 12 Mo de pièces jointes : la lecture doit
     être tolérante, et un incident sur Drive ne doit jamais faire perdre la demande. */
  async function requete(adresse, options) {
    var ctrl = typeof AbortController !== "undefined" ? new AbortController() : null;
    var lourd = options && options.body && String(options.body).length > 200000;
    var minuteur = setTimeout(function () { if (ctrl) ctrl.abort(); }, lourd ? 180000 : 60000);
    try {
      ...
      if (e && e.name === "AbortError") throw new Error("le service n'a pas répondu dans le délai imparti");
      throw e;
    } finally {
  ...
  var Stockage = {
    ...
    deposer: async function (demande, courriels) {
      ...
      /* Une seconde tentative avec la même clé de dépôt : le serveur reconnaît
         un dépôt déjà abouti et renvoie sa référence au lieu d'un doublon.
         Répare le cas — observé — où l'écriture réussit mais la réponse
         n'atteint pas le navigateur. */
      var charge = { action: "deposer", demande: demande, courriels: courriels || [] };
      try {
        return await ecrireApi(charge);
      } catch (premiere) {
        if (!demande.cle) throw premiere;
